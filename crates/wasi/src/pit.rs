use crate::{StoreT, ContextTrait, log::Module, Dev, bus::BusData};
...
impl PIT {
    ...
    pub fn init(&mut self) {
        self.store.io_mut().map(|io| {
            ...
            io.register_read8(0x40, 
                Dev::Emulator(self.store.clone()), 
                |dev: &Dev, _addr: u32| {
                    dev.pit_mut().map_or(0, |pit| {
                        pit.counter_read(0)
                    })
                }
            );
            io.register_read8(0x41, 
                Dev::Emulator(self.store.clone()), 
                |dev: &Dev, _addr: u32| {
                    dev.pit_mut().map_or(0, |pit| {
                        pit.counter_read(1)
    ...
    pub fn timer(&mut self, now: f64, no_irq: bool) -> f64 {
        ...
        if !no_irq {
            if self.counter_enabled[0] > 0 && self.did_rollover(0, now) {
                ...
                dbg_log!(Module::PIT, "pit interrupt. new value: {}", self.counter_start_value[0]);
    ...
    fn did_rollover(&self, i: u8, now: f64) -> bool {
        ...
        if diff < 0.0 {
            ...
            dbg_log!(Module::PIT, "Warning: PIT timer difference is negative, resetting (timer {})", i);
            return true;
        }
    ...
    fn get_counter_value(&mut self, i: u8, now: f64) -> u16 {
        ...
        dbg_log!(Module::PIT, "diff={} dticks={} value={} reload={}", diff, diff_in_ticks, value, self.counter_reload[i]);

        let reload = self.counter_reload[i];

        if value >= reload {
            dbg_log!(Module::PIT, "Warning: Counter{} value {} is larger than reload {}", i, value, reload);
            value %= reload;
    ...
    fn port43_write(&mut self, reg_byte: u8) {
        ...
        if read_mode == 0 {
            ...
            self.counter_latch_value[i] = if value > 0 { 
                value - 1
            } else {
                0
            };
